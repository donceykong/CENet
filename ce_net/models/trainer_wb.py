...
class Trainer:
    def __init__(self, ARCH, DATA, dataset_name, datadir, logdir, path=None):
        ...
        # KITTI-360 still uses (scans, labels) tuples per split; normalize here.
        # MCD normalization happens upstream in scripts/train.py so DATA["split"]
        # already carries per-sensor shards.
        if dataset_name == "KITTI-360" and "sequences" in self.DATA and isinstance(self.DATA.get("split"), list):
            ...
            print(f"KITTI-360 split: train={n_train}, valid={n_valid}, test={n_test} scans")
