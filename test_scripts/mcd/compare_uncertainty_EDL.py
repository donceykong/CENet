...
def compare_gt_inferred_map(dataset_path, seq_name, label_config,
    ...
    EDL outputs (used as follows):
    - confidence_scores: Vacuity (K/S) → primary uncertainty for calibration, filtering, and coloring (no variance needed).
    - multiclass_confidence_scores: Dirichlet predictive probs (alpha/S) → predictions (argmax), second-best class, and confidence (max p).
    Variance of probs is only a fallback when confidence_scores are missing.
    view_mode: "all" | "correct" | "incorrect" — show all inferred points, or only correct/incorrect.
    view_variance: If True, color by uncertainty (vacuity when EDL, else variance fallback) (Viridis: yellow=uncertain, dark=confident).
    infer_subdir: Subdir under inferred_labels/ (e.g. cenet_mcd_EDL) where inference wrote outputs.
    """
    ...
    base_infer = os.path.join(root_path, "inferred_labels", infer_subdir)
    multiclass_dir = os.path.join(base_infer, "multiclass_confidence_scores")
    confidence_dir = os.path.join(base_infer, "confidence_scores")
    ...
    all_inf_variance = []  # variance of Dirichlet probs (only used for fallback and optional report when not EDL)
    all_inf_uncertainty = []  # vacuity when EDL, else variance-based fallback
    all_inf_maxprob = []  # max Dirichlet predictive prob (confidence) per point
    use_vacuity = os.path.isdir(confidence_dir)  # EDL: use vacuity; no need for variance for uncertainty
    ...
    for pose_num in tqdm(sorted_pose_indices, desc="Loading scans", unit="scan"):
        ...
        conf_path = os.path.join(confidence_dir, bin_file) if use_vacuity else None
        ...
        try:
            ...
            # Dirichlet predictive probs (alpha/S) from EDL inference: (N, n_classes), rows sum to 1
            ...
            variances = np.var(multiclass_probs.astype(np.float32), axis=1)  # for optional report/plot
            # Uncertainty: Dirichlet vacuity (K/S) when EDL outputs exist; only fall back to variance when no confidence_scores
            if conf_path is not None and os.path.exists(conf_path):
                vacuity = read_bin_file(conf_path, dtype=np.float16)
                if len(vacuity) == n_points:
                    scan_uncertainty = np.asarray(vacuity, dtype=np.float32)  # use vacuity, not variance
                else:
                    max_var = (n_classes - 1) / (n_classes ** 2) if n_classes > 1 else 1.0
                    scan_uncertainty = 1.0 - np.clip(variances.astype(np.float32) / max_var, 0.0, 1.0)
            else:
                max_var = (n_classes - 1) / (n_classes ** 2) if n_classes > 1 else 1.0
                scan_uncertainty = 1.0 - np.clip(variances.astype(np.float32) / max_var, 0.0, 1.0)
            uncertainty = scan_uncertainty if view_variance else None
            # Confidence from Dirichlet predictive probs: max p (best single-class probability)
            max_prob = np.max(multiclass_probs.astype(np.float32), axis=1)
        except Exception as e:
        ...
        if max_distance is not None and max_distance > 0:
            ...
            scan_uncertainty = scan_uncertainty[dist_mask]
            max_prob = max_prob[dist_mask]
        ...
        if voxel_size is not None and voxel_size > 0 and len(world) > 0:
            ...
            scan_uncertainty = scan_uncertainty[idx]
        ...
        all_inf_uncertainty.append(scan_uncertainty)
        all_inf_maxprob.append(max_prob)
    ...
    inf_maxprob = np.concatenate(all_inf_maxprob) if all_inf_maxprob else None
    ...
    assert inf_uncertainty is not None and len(inf_uncertainty) == len(inf_points)
    assert inf_maxprob is not None and len(inf_maxprob) == len(inf_points)

    # Primary uncertainty: EDL vacuity (from confidence_scores) when available, else variance-based
    uncertainty_all = inf_uncertainty
    if use_vacuity:
        print("Using EDL vacuity (confidence_scores) as uncertainty.")
        print("Mean confidence (max p from Dirichlet): %.4f" % float(np.mean(inf_maxprob)))
    else:
        print("Using variance of class probabilities as uncertainty (no confidence_scores found).")
        print("Mean confidence (max p): %.4f" % float(np.mean(inf_maxprob)))
    ...
    if bin_accs:
        ...
        ax_cal.set_xlabel("Mean vacuity (bin)" if use_vacuity else "Mean uncertainty (bin)")
        ax_cal.set_ylabel("Accuracy (%)")
        ax_cal.set_title("Accuracy by uncertainty bin (well-calibrated: curve decreases)" + (" [vacuity]" if use_vacuity else ""))
    ...
    if use_vacuity:
        avg_unc = float(np.mean(inf_uncertainty[mask]))
        print(f"Average vacuity ({view_mode}): {avg_unc:.6f}")
        unc_subset = inf_uncertainty[mask]
        fig, ax = plt.subplots(figsize=(8, 5))
        ax.hist(unc_subset, bins=80, color="steelblue", edgecolor="white", alpha=0.85)
        ax.axvline(avg_unc, color="coral", linewidth=2, label=f"Mean = {avg_unc:.4f}")
        ax.set_xlabel("Vacuity (K/S)")
        ax.set_ylabel("Count")
        ax.set_title(f"Vacuity distribution (view={view_mode}, n={n_show})")
        ax.legend()
    else:
    ...
    if view_variance and inf_uncertainty is not None:
        ...
        colors = scalar_to_viridis_rgb(unc, normalize_range=True)  # full range (vacuity or variance) -> viridis
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--variance", action="store_true",
                        help="Color by uncertainty (vacuity when EDL; else variance fallback; Viridis: yellow=uncertain, dark=confident)")
    parser.add_argument("--infer-dir", type=str, default="cenet_mcd_EDL",
                        help="Inference output subdir under inferred_labels/ (default: cenet_mcd_EDL)")
